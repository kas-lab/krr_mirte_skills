...
def is_entity_in_room(entity_state, corners):
    ...
    for i in range(4):
        ...
        cross = edge_vector[0] * point_vector[1] - edge_vector[1] * point_vector[0]
...
class GetObjectsInRoomNode(Node):
    ...
    def which_room(self, entity_state):
        room_checklist = [(room[0], is_entity_in_room(entity_state, room[1])) for room in list(self.rooms.items())]

        if sum([is_in_room for _, is_in_room in room_checklist]) != 1: return None #In no or multiple rooms.

        return next((name for name, is_in_room in room_checklist if is_in_room))
    ...
    def get_objects_callback(self, _, response):
        ...
        grabable_entities = [entity_name for entity_name in entity_names if entity_name.startswith(self.GRABABLE_ENTITY_PREFIX)]
        ...
        robot_entity_state = self.send_request_async(self.get_entity_cli,self.ent_req)
